...
CONFIG_TMP=$(mktemp "${TMPDIR:-/tmp}/msa-avd-config.XXXXXX")
...
if git -C "$ROOTAVD_DIR" apply --check "$ROOT/patches/rootavd-msa.patch" 2>/dev/null; then
  git -C "$ROOTAVD_DIR" apply "$ROOT/patches/rootavd-msa.patch"
fi
